...
bootstrap <- function(data, direct_fun, ensemble_fun, sample.percentage, iterations,topk,params){
  ...
  if(Sys.info()['sysname']=='Windows'){
    ...
    lib_dir=paste0(lib_dir,ver)
      ...
      library(corpcor,lib.loc = lib_dir)
      library(space,lib.loc = lib_dir)
      library(pcalg,lib.loc = lib_dir)
      library(DT,lib.loc = lib_dir)
      library(parallel,lib.loc = lib_dir)
      library(ParallelPC,lib.loc = lib_dir)
      })
  }else{
